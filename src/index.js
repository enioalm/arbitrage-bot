...
let {
  apiKey, apiSecret, amount,amountCurrency ,amountBtc,amountCripto ,initialBuy, minProfitPercent, intervalSeconds, playSound, simulation,
...
const checkBalances = async () => {
  ...
  if (nAmount > Number(amountBalance)) {
    ...
    amount = Number(amountBalance);
    handleMessage(`Novo Valor para Arbitragem e de:  ${amount} `);
  }
};
...
const atualizaBalances = async () => {
  ...
  let amountBalanceNew = isQuote ? BRL : BTC;

  if (mAmount > Number(amountBalance)){
    handleMessage(
      `Montante ${amountBtc} é maior que o do usuário ${isQuote ? 'BRL' : 'BTC'} Saldo de ${amountBalanceNew}`);
  }else{
    amountBtc = Number(amountBalanceNew);
    recompra =true;
  }
};
        ...
        if(firstLeg && secondLeg){
          await atualizaBalances();
            if(recompra){
              ...
              handleMessage(`[${tradeCycleCount}] Foi Vendido os Bitcoins adquiridos na Arbritagem: ${amountBtc}`);
              await checkBalances();
            }
        }
      } catch (error) {
